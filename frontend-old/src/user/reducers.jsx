import { createImageURL } from "../imports.jsx";
...
export default {
  ...
  USER_PROFILE_UPDATED: (state, payload) => ({
    ...
    infoMsg: `"Successfully saved user data for" ${payload.firstName} ${payload.lastName}`,
  }),
  ...
  USER_PROFILE_IMAGE_UPDATED: (state, payload) => {
    ...
    return {
      ...
      infoMsg: `"Successfully updated user image for" ${state.signedInUserData.firstName} ${state.signedInUserData.lastName}`,
    };
  },
  ...
  SIGNED_IN: (state, payload) => ({
    ...
    signedInUserData: {
      ...
      lastName: payload.lastName,
      imageData:
        payload.img && payload._attachments[payload.img]
          ? createImageURL(payload._attachments[payload.img])
          : "assets/img/userm.png",
    },
    infoMsg: `${payload.firstName} ${payload.lastName} "is signed in"!`,
  }),

  // reset state after successful sign out
  SIGNED_OUT: (state, payload) => ({
    ...initialState,
    infoMsg: `${payload.firstName} ${payload.lastName} "is signed out"!`,
  }),
};
