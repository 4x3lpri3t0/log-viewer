...
exports.Db = function Db(options) {
  ...
  if (this.db && typeof this.db !== "string")
    throw new Error("db property must be a string");
...
mixin(exports.Db.prototype, {
  ...
  allDocs: function (query, callback) {
    var opt = { method: "GET", path: "/_all_docs" },
      body,
      t = typeof query;
    ...
    this.request(opt, function (err, result, response) {
      if (callback) {
        ...
        var docs = {};
        for (
          var i = 0, L = result.rows.length;
          i < L && (row = result.rows[i]);
          i++
        )
  ...
  request: function (options, callback) {
    ...
    if (this.auth) {
      ...
      opt.headers.Authorization =
        "Basic " + base64_encode(this.auth.username + ":" + this.auth.password);
    }
    // Setup headers
    if (!opt.headers.Host) opt.headers.Host = this.host;
    if (!opt.headers.Connection)
      opt.headers.Connection =
        this.connectionPool.keep < 1 ? "Close" : "Keep-Alive";
    // Setup path
    ...
    if (opt.query) {
      for (var k in opt.query)
        if (k !== "stale") opt.query[k] = JSON.stringify(opt.query[k]);
      opt.path += "?" + querystring.stringify(opt.query);
    }
    // Setup body
    if (opt.body && (opt.method === "PUT" || opt.method === "POST")) {
      if (typeof opt.body !== "string") opt.body = exports.jsonEncode(opt.body);
    ...
    var onconn = function (err, conn) {
      ...
      var onConnClose = function (hadError, reason) {
        if (hadError && callback && !cbFired) {
          cbFired = true;
          callback(new Error(reason || "Connection error"));
        }
        ...
        res.addListener("end", function () {
          ...
          if ("error" in result) {
            if (!cbFired && callback) {
              cbFired = 1;
              var err = new Error(
                "CouchDb " + result.error + ": " + (result.reason || "?")
              );
      ...
      timeoutId = setTimeout(function () {
        ...
        if (callback && !cbFired) {
          ...
          callback(
            new Error(
              req
                ? "CounchDB connection timeout"
                : "CouchDB connection pool timeout"
            )
          );
        }
      }, opt.timeout);
  ...
  _putOrPost: function (method, options, body, callback) {
    ...
    if (typeof body === "function") {
      ...
      throw new Error(
        "Conflicting arguments: both options.body and the body argument are set"
      );
    } else {
      options.body = body;
    }
...
Pool.prototype.cancelGet = function (callbackToCancel) {
  var i = this.getQueue.indexOf(callbackToCancel),
    found = i !== -1;
...
HTTPConnectionPool.prototype.create = function () {
  ...
  if (this.secure) {
    ...
    conn.setSecure(
      "X509_PEM",
      this.secure.ca_certs,
      this.secure.crl_list,
      this.secure.private_key,
      this.secure.certificate
    );
  }
  conn._onclose = function (hadError, reason) {
    ...
    if (hadError)
      self.emit(
        "error",
        new Error("Connection error" + (reason ? ": " + reason : ""))
      );
    try {
      conn.removeListener("close", conn._onclose);
    } catch (e) {}
  };
