import { utf8ToHex } from "../imports.jsx";
...
const couchdbErrMessages = {
  ...
  forbidden: "You are not allowed to authenticate with these credentials",
  unauthorized: "Invalid user credentials",
  authentication_error: "Invalid user credentials",
  not_found: "Problem with user account, please contact admins",
};
...
export const signUp = (props) => (dispatch) => {
  ...
  db.remote
    .signUp(props.username, props.password, {
      metadata: {
...
const retreiveUserData = (username) => async (dispatch) => {
  ...
  db.remote
    ...
    .then((userMetaData) =>
      db.user.local
        .get("profileImg", { attachments: true, binary: true })
        .then((profileImgDoc) => (profileImg = profileImgDoc))
        .catch(
          async (err) =>
            err.reason === "missing" && (await db.user.local.put(profileImg))
        )
        .finally(() => {
          dispatch({
            type: SIGNED_IN,
...
export const updateProfile = (props) => (dispatch) => {
  ...
  if (props.firstName) userData.firstName = props.firstName;

  if (props.lastName) userData.lastName = props.lastName;

  db.remote
    .putUser(props.username, { metadata: userData })
    .then(() =>
